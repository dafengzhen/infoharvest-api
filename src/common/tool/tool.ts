export const mergeAndDistinctArrays = (array1: any[], array2: any[], mergeProperties: string[]) => {
  ...
  return mergedArray.reduce((previousValue, currentValue) => {
    ...
    if (existingItemIndex !== -1) {
      ...
      for (const prop of mergeProperties) {
        if (currentValue.hasOwnProperty(prop) && currentValue[prop] !== undefined) {
...
export const mergeObjects = (obj1: object, obj2: object, properties: string[]) => {
  const newObj = { ...obj1 };
